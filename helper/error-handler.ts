import { AppError, ErrorStatusCode } from "./app-error";
import { log, shortAxiosError } from "./logger";
import { AxiosError } from "axios";

export function errorHandler(
	caughtError: Error | AppError | AxiosError
): undefined {
	let error = caughtError;
	const axiosError =
		(caughtError as AxiosError).isAxiosError &&
		shortAxiosError(caughtError as AxiosError);
	...
	if (!(error instanceof AppError)) {
		 ...
		 */
		log(
			"This error was not generated by us. We should extract the statusCode, name and message here"
		);
	...
	switch (error.statusCode) {
		case 400:
			throw `[${error.statusCode}] ${error.name ? `${error.name}. ` : ""}${
		...
		case 409:
			throw `[400] ${error.name ? `${error.name}. ` : ""}${
		...
		case 503:
			throw `[500] ${error.name ? `${error.name}. ` : ""}${
